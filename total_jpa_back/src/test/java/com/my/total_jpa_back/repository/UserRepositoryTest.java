...
//    }
    ...
    void sliceTest() {
        ...
        List<Users> users = result.getContent();  // getContent로 받아줘야한다.
        // 전체 행 수
...
//        log.info("전체 페이지 : " + result.getTotalPages());
    ...
    // 페이징 처리에 사용되는 클래스 : Pageable(interface)
    // 전체 회원 자료를 10개 묶음으로 페이징 처리
    // PageRequest.of
...
//            log.info("name = {}", users.get(i).getName());
    ...
    void findByLikeColorAndGender() {
        ...
        for (Users user : users) {
            log.info("name = {}, gender= {}, color = {}", user.getName(), user.getGender(), user.getLikeColor());
        }
    }
}